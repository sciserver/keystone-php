<?php
  function show_user_info_v3($token_id, $user_info, $linked_user_id) 
  {
...
EOT;
  ...
  function show_user_info_v2($token_id, $user_info, $linked_user_id) 
  {
    /* Prints various properties of the user_info
    object, obtained from the token validation request. 
    We also might want to print token_id, which is not
    included in the user_info. */
    $user_name = $user_info->access->user->username;
    $user_id = $user_info->access->user->id;
    $roles_array = $user_info->access->user->roles;
    $roles = "";
    foreach ($roles_array as &$role) 
    {
      $roles = $roles.$role->name."</br>";
    ...
    //$domain = $user_info->token->user->domain->name;
    $project = $user_info->access->token->tenant->name;
    $issued_at = $user_info->access->token->issued_at;
    $expires_at = $user_info->access->token->expires;
